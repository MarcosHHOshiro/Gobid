...
const getBidsByProductsId = `-- name: GetBidsByProductsId :many
SELECT id, product_id, bidder_id, bid_amount, created_at FROM bids 
WHERE product_id = $1
ORDER BY bid_amount DESC
`

func (q *Queries) GetBidsByProductsId(ctx context.Context, productID uuid.UUID) ([]Bid, error) {
	rows, err := q.db.Query(ctx, getBidsByProductsId, productID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
